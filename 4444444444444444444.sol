...
contract Korilakkuma is IERC20, TokenEvents, ReentrancyGuard {
    ...
    address internal development_receiver = 0x725252Fd175AB01078B86e8D14bBf1E40B56D078; 
    address internal marketing_receiver = 0x725252Fd175AB01078B86e8D14bBf1E40B56D078;
    address internal liquidity_receiver = 0x725252Fd175AB01078B86e8D14bBf1E40B56D078;
    ...
    function setUniswapV2Router(IUniswapV2Router02 _uniswapV2Router) external onlyAdmins {
    ...
    function stakeLP(uint amt, uint _lpIndex, address _referrer)
        external
        synchronized
    {
    ...
    function unstakeLP(uint _lpIndex)
        external
        synchronized
    {
    ...
    function HarvestKorilakkuma(uint _lpIndex)
        external
        synchronized
    {
    ...
    function NewHalvening()
        ...
        synchronized
    {   
        require(block.timestamp.sub(halveningTimestamp) >= DAYSECONDS.mul(halveningDays), "cannot call halvening yet");
        halveningDays += 7; //increase period by 1 week every halvening
        halveningTimestamp = block.timestamp;
        halvening = halvening * 2;
    }

    function BurnKorilakkuma(uint amt)
        external
        synchronized
    {
        ...
        _balances[cakeKORILAKKUMAETH] = _balances[cakeKORILAKKUMAETH].sub(amt, "ERC20: burn amount exceeds balance");
        ...
        returns(uint)
    {
        ...
        uint minsPast = block.timestamp.sub(farmer[_user].stakeStartTimestamp).div(MINUTESECONDS);
        if(minsPast >= 1){
    ...
    function calcHarvestRewards(address _user, uint _lpIndex)
        ...
        returns(uint)
    {   
        return ((lpFrozenBalances[_user][_lpIndex].mul(globalApy).div(lpApy[lpAddresses[_lpIndex]])).mul(minsPastStakeTime(_user, _lpIndex)).div(halvening));
    }
    
    //returns amount of minutes past since lp stake start
    function minsPastStakeTime(address _user, uint _lpIndex)
        public
        view
        returns(uint)
    {
        ...
        uint minsPast = block.timestamp.sub(lpStakeStartTimes[_user][_lpIndex]).div(MINUTESECONDS);
        if(minsPast >= 1){
            return minsPast;// returns 0 if under 1 min passed
        }
        else{
        ...
        returns(bool)
    {
        if(farmer[_user].stakeStartTimestamp == 0){
            ...
            return farmer[_user].stakeStartTimestamp.add((DAYSECONDS).mul(MINSTAKEDAYLENGTH)) <= block.timestamp;             
        }
    }
        ...
        returns(bool)
    {
        if(lpStakeStartTimes[_user][_lpIndex] == 0){
           ...
           return lpStakeStartTimes[_user][_lpIndex].add(MINUTESECONDS) <= block.timestamp; 
        }
    }

    function setKORILAKKUMAETHpool(address _lpAddress)
        external
        onlyAdmins
    {
   ...
   function setTaxRate(uint256 newTaxRate) external onlyAdmins {
        ...
        onlyAdmins
    {
        ...
        for(uint i = 0; i < lpAddresses.length; i++){
            require(_tokenAddress != lpAddresses[i], "this token is vital to the korilakkuma.finance ecosystem, you cannot withdraw this token!!!");
        }
        //ensure token address does not match this contract
        require(_tokenAddress != address(this), "this token is vital to the korilakkuma.finance ecosystem, you cannot withdraw this token!!!");
        //create contract
